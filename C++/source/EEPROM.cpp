...
void eeprom::burst_write(uint8_t* send_data)
{
	...
	page_number=last_address/PAGE_SIZE;
	remain_page=PAGE_SIZE-(last_address%PAGE_SIZE);
...
bool eeprom::burst_read(uint16_t read_address,uint32_t nbyte,uint8_t* receive_data)
{
	
	if(!check_address(read_address)) return false;
	//maybe you can wite better function
	read_address+=ID_BYTE;
	page_read(read_address,nbyte,receive_data);
	return true;
}
bool eeprom::check_address(uint16_t address)
{
	uint32_t virtual_address;
	virtual_address=calculate_next_address(last_address);
...
bool eeprom::i2c_page_write(uint16_t address, uint8_t data_bytes , uint8_t* data)
{
	...
	switch(check_address(address))
	{
		...
		case 1:
			...
			switch(ADDRESS_BYTES)
			{
				...
				break;
				case 16:
					I2C_SendData(EEPROM_I2C,static_cast<uint8_t>((address&0xff00)>>8));
					counter=0;
					while(!I2C_CheckEvent(EEPROM_I2C,I2C_EVENT_MASTER_BYTE_TRANSMITTED))
					{
						if(flag_time_overflow( &counter , MaxTime )) return 0;
					}
					I2C_SendData(EEPROM_I2C,static_cast<uint8_t>(address&0x00ff));
					counter=0;
					while(!I2C_CheckEvent(EEPROM_I2C,I2C_EVENT_MASTER_BYTE_TRANSMITTED))
					{
						if(flag_time_overflow( &counter , MaxTime )) return 0;
...
bool eeprom::i2c_page_read(uint16_t address, uint8_t data_bytes , uint8_t* receive)
{
	uint32_t counter,MaxTime=2000;
	switch(check_address(address))
	{
...
bool eeprom::i2c_byte_write(uint16_t address,uint8_t data)
{
	uint32_t counter,MaxTime=2000;
	switch(check_address(address))
	{
...
bool eeprom::i2c_byte_read(uint16_t address,uint8_t* data)
{
	uint32_t counter,MaxTime=2000;
	switch(check_address(address))
	{
...
void eeprom::byte_write(uint16_t address,uint8_t data)
{
	bool done=false;
	do
	{
...
void eeprom::byte_read(uint16_t address,uint8_t* data)
{
	bool done=false;
	do
	{
...
void eeprom::page_write(uint16_t address, uint8_t data_bytes , uint8_t* data)
{
	bool done=false;
	do
	{
...
void eeprom::page_read(uint16_t address, uint8_t data_bytes , uint8_t* receive)
{
	bool done=false;
	do
	{
...
bool eeprom::id_write(uint16_t id_address,uint32_t id)
{
	int byte_id=ID_BYTE;
	uint32_t counter,MaxTime=2000;
	I2C_GenerateSTART(EEPROM_I2C,ENABLE);
	...
	switch(ADDRESS_BYTES)
	{
		...
		break;
		case 16:
			I2C_SendData(EEPROM_I2C,static_cast<uint8_t>((id_address&0xff00)>>8));
			counter=0;
			while(!I2C_CheckEvent(EEPROM_I2C,I2C_EVENT_MASTER_BYTE_TRANSMITTED))
			{
				if(flag_time_overflow( &counter , MaxTime )) return 0;
			}
			I2C_SendData(EEPROM_I2C,static_cast<uint8_t>(id_address&0x00ff));
			counter=0;
			while(!I2C_CheckEvent(EEPROM_I2C,I2C_EVENT_MASTER_BYTE_TRANSMITTED))
			{
				if(flag_time_overflow( &counter , MaxTime )) return 0;
...
uint32_t eeprom::id_read(uint16_t id_address)
{
	int byte_id=ID_BYTE;
...
uint16_t eeprom::find_address(void)
{
...
uint16_t eeprom::calculate_next_address(uint16_t address)
{
	return (address+DATA_SIZE+ID_BYTE);
}
bool eeprom::flag_time_overflow( uint32_t* i , uint32_t timeout )
{
	bool overflow = 0;
	*i+=1;
	if( *i >= timeout )
...
void eeprom::delay_ms( uint32_t ms )
{
